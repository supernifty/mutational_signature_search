...
def filter_mutect2(sample, dp, af, use_bam_depth):
...
def filter_strelka(sample, dp, af, use_bam_depth):
...
def main(genome, signatures, vcfs, dps, afs, context_cutoff, caller, tags, use_bam_depth, doublets, indels, just_indels):
  ...
  for vcf in vcfs:
    ...
    for dp in dps:
      for af in afs:
        # calculate counts
        logging.info('calculating counts for %s with dp %i af %.3f caller %s...', vcf, dp, af, caller)
        variant_filter = None

        if caller == 'mutect2':
          variant_filter = filter_mutect2(sample, dp, af, use_bam_depth)

        elif caller == 'strelka':
          variant_filter = filter_strelka(sample, dp, af, use_bam_depth)
    ...
    for dp in dps:
      for af in afs:
        ...
        logging.info('calculating signature for %s with dp %i af %.3f with %i variants', vcf, dp, af, totals)
        out = outs[idx]
        idx += 1
...
if __name__ == '__main__':
  ...
  main(args.genome, args.signatures, args.vcfs, args.dps, args.afs, args.context_cutoff, args.caller, args.tags, args.use_bam_depth, args.doublets, args.indels, args.just_indels)
