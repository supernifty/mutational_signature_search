...
COUNT_SPLIT = 0.05
X_HIGHLIGHT_COLOUR=['#ff0000', '#ff8000', '#4040ff']
...
def plot_hist(data_fh, samples, x, target, filters, title, logx, highlight, error_plot, count_plot, split_count, signature_ids, min_signature_val, x_highlight=None, height=8, width=12, fontsize=12, dpi=300, linewidth=0.5, legend_cols=1):
    ...
    if samples is None or row['Sample'] in samples:
      ...
      if ok:
        ...
        if signature_ids is None:
          signature_ids = [k for k in colors.keys() if k in row]
          logging.debug('plotting %s', signature_ids)
  ...
  if error_plot and not count_plot: # just error plot
    ...
    ax_err.tick_params(axis='x', labelsize=fontsize)
    ax_err.tick_params(axis='y', labelsize=fontsize)
    ...
    if split_count:
      ...
      ax_count_top.tick_params(axis='x', labelsize=fontsize)
      ax_count_top.tick_params(axis='y', labelsize=fontsize)
      ax_count_bottom = fig.add_subplot(grid[-10:, :], sharex=ax) # give to half of variant count plot
      ax_count_bottom.tick_params(axis='x', labelsize=fontsize)
      ax_count_bottom.tick_params(axis='y', labelsize=fontsize)
    else:
      ...
      ax_count.tick_params(axis='x', labelsize=fontsize)
      ax_count.tick_params(axis='y', labelsize=fontsize)
  ...
  for sig in range(0, len(signature_ids)):
    ...
    if (highlight is None or label not in highlight) and max(ys) < min_signature_val:
      ...
      ax.fill_between(xs, base, ys_cumulative, color=color, alpha=0.2)
    else:
  ...
  if x_highlight is not None:
    for x_i, x_h in enumerate(x_highlight):
      ax.axvline(x_h, color=X_HIGHLIGHT_COLOUR[x_i % len(X_HIGHLIGHT_COLOUR)], ymin=0, ymax=1)

  ax.set_ylabel('Signature proportion', fontsize=fontsize)
  ...
  if title is None:
    ax.set_title('Somatic mutational signatures detected by {} for {} ({})'.format(x, ' '.join(samples or []), ' '.join(filters or [])))
  else:
  ...
  if count_plot:
    ...
    if split_count:
      ...
      ax_count_top.plot(xs, ys, 'k-', linewidth=linewidth)
      ax_count_bottom.plot(xs, ys, 'k-', linewidth=linewidth)
      ...
      kwargs = dict(transform=ax_count_top.transAxes, color='k', clip_on=False, linewidth=linewidth)
      ...
      ax_count_bottom.set_xlabel(LABELS[x], fontsize=fontsize)
      ax_count_bottom.set_ylabel('Variants', fontsize=fontsize)
    else:
      ax_count.plot(xs, ys, 'k-', linewidth=linewidth)
      ax_count.fill_between(xs, 0, ys, color='#a0a0a0')
      ax_count.grid(True)
      ax_count.set_ylabel('Variants', fontsize=fontsize)
      ax_count.set_xlabel(LABELS[x], fontsize=fontsize)
...
if __name__ == '__main__':
  ...
  parser.add_argument('--x_highlight', required=False, type=float, nargs='+', help='vertical line at this value')
  parser.add_argument('--logx', action='store_true', help='log x')
  parser.add_argument('--error_plot', action='store_true', help='include error plot')
  ...
  parser.add_argument('--linewidth', required=False, default=0.5, type=float, help='line widths')
